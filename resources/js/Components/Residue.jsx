...
function Residue({ id, datas, errors }) {
    ...
    useEffect(() => {
        if (datas) {
            // Extraer la información de residue desde datas
            const residue = extractData(datas, 'residue', null, {});

            // Configurar categoría seleccionada
            setSelectedCategory(residue.selected || null);

            // Manejo de "Custom Clause"
            if (residue.selected === "Custom Clause") {
                setSelectedCategory("Custom Selection");
                setSelectedOption(residue.selected);
                setCustom(true);
                setSpecific(false);
                setClauseValue(residue.clause || "");
                setTable_dataBequest([]); // Aseguramos que no haya datos de beneficiarios
            }

            // Manejo de "Specific Beneficiaries"
            else if (residue.selected === "Specific Beneficiaries") {
                setSelectedCategory("Custom Selection");
                setSelectedOption(residue.selected);
                setSpecific(true);
                setCustom(false);
                setTable_dataBequest(residue.beneficiary || []);
                setAvailableShares(
                    100 - (residue.beneficiary?.reduce((sum, item) => sum + item.shares, 0) || 0)
                );

                // Verificar si hay organizaciones en los beneficiarios
                const isOrg = residue.beneficiary?.some(b => b.isOrganization) || false;
                setIsOrganization(isOrg);
            }

            // Manejo de "Bloodline Selection"
            else if (residue.selected?.startsWith("Have the residue go to")) {
            ...
            // Guardar el estado del objeto global `obj`
            obj = { selectedOption: residue.selected };
            setSelected(obj);
        }
    }, [datas]);
